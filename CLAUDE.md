...
- **RAG**: `src/bailian_rag_demo/rag/base.py` defines `KnowledgeBase` ABC. Stage 1 implementation: `bailian_kb.py` (BaiLianKB). Retrieved chunks are folded into the user turn's content before being handed to AgentScope; RAG stays swappable without touching agent orchestration.
